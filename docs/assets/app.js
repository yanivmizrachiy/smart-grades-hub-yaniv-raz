...
function counts(){
  const c={Z:0,H:0,T:0};
  ST.students.forEach(s=>{ const g=gradeKey(s.class); if(g&&c[g]!=null) c[g]++; });
  els.z.textContent=c.Z; els.h.textContent=c.H; els.t.textContent=c.T;
}
...
function renderTrack(layer,track,teacherLabel){
  ...
  info.innerHTML=`<span class="badge">${rows.length} תלמידים — ${teacherLabel||""}</span>`;
  const table=document.createElement("table");
  table.className="table";
  table.innerHTML=`
    <thead><tr>
...
async function load(){
  try{
    ...
    try{ ST.teachers=await fetch("data/teachers.json?ts="+Date.now()).then(r=>r.ok?r.json():null); }catch(_){ ST.teachers=null; }
    counts();
    const selected=localStorage.getItem("lastLayer");
    if(selected && ["Z","H","T"].includes(selected)){ renderLayer(selected); }
...
window.addEventListener('beforeinstallprompt',(e)=>{
  ...
  if(btn){
    ...
    btn.onclick=async()=>{
      ...
      finally{ deferredPrompt=null; btn.disabled=false; btn.style.display='none'; }
    };
  }
});
window.addEventListener('appinstalled',()=>{ const b=document.getElementById('installPWA'); if(b) b.style.display='none'; });
